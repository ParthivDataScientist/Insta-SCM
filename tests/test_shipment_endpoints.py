...
class TestGoogleSheetWebhook:
    ...
    def test_webhook_stores_show_date_and_returns_structured_summary(self, client):
        payload = {
            "rows": [
                {
                    "row_number": 2,
                    "master_awb": "1234567890",
                    "child_awb": "",
                    "ship_to_location": "Mumbai, IN",
                    "client_name": "Acme",
                    "show_date": "2026-05-10",
                    "show_city": "Mumbai Expo",
                    "courier": "DHL",
                }
            ]
        }
        ...
        assert data["tracking_mode"] == "metadata_only"

        list_resp = client.get("/api/v1/shipments/")
        shipment = next(r for r in list_resp.json() if r["tracking_number"] == "1234567890")
        assert shipment["show_date"] == "2026-05-10"

    def test_webhook_reports_exact_row_failure(self, client):
        payload = {
            "rows": [
                {
                    "row_number": 12,
                    "master_awb": "",
                    "child_awb": "",
                    "client_name": "Missing Tracking Client",
                    "show_date": "2026-05-10",
                },
                {
                    "row_number": 13,
                    "master_awb": "",
                    "child_awb": "JD014600012565061255",
                    "courier": "DHL",
                },
            ]
        }
        ...
        assert data["failures"][0]["row_number"] == 12
        assert data["failures"][0]["reason"] == "Missing master_awb and child_awb"
        assert data["failures"][1]["row_number"] == 13
        assert data["failures"][1]["reason"] == "Child shipment is missing a master_awb relationship"
